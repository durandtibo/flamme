...
def create_section_template() -> str:
    ...
    return """<h{{depth}} id="{{id}}">{{section}} {{title}}
           ...
           temporal distribution of null values. The column
           <em>{{column}}</em> is used as the temporal column.
...
def create_temporal_null_figure(
    ...
    return Template("""<div class="container-fluid text-center"> <div class="row
        align-items-start"> {{columns}} </div> </div>""").render(
...
def create_table_section(
    ...
    return Template("""<details> <summary>[show statistics per temporal
        ...
        statistics for each period of column {{column}}.
...
def create_temporal_null_table(
    ...
    return Template("""<table class="table table-hover table-responsive w-auto" >
        <thead class="thead table-group-divider"> <tr> <th colspan="6"
        style="text-align: center">column: {{column}}</th> </tr> <tr>
        <th>period</th> <th>number of null values</th> <th>number of
        non-null values</th> <th>total number of values</th>
        <th>percentage of null values</th> <th>percentage of non-null
        values</th> </tr> </thead> <tbody class="tbody table-group-
...
def create_temporal_null_table_row(label: str, null: int, total: int) -> str:
    ...
    """
    non_null = total - null
    return Template("""<tr> <th>{{label}}</th> <td {{num_style}}>{{null}}</td> <td
        {{num_style}}>{{non_null}}</td> <td {{num_style}}>{{total}}</td>
        <td {{num_style}}>{{nulls_pct}}</td> <td
        {{num_style}}>{{non_null_pct}}</td> </tr>""").render(
        {
